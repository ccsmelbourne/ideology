---
title: "Kashima_etal_2020_friendly"
author: "Elle Pattenden"
output: html_document
---

# Ideology, Communication, and Polarization Simulations 

This document contains code to run the simulations and produce the plots from Kashima et al. (2020), *Ideology, Communication, and Polarization*. 

Two key parameters are varied in the simulations. 

1. Agent **Cognitive Type**, which creates a population of agents with an:
  + ideological filter and ideological ego-involvement (*type = 1*)
  + ideological filter and non-ideological ego-involvement (*type = 2*)
  + unbiased filter and ideological ego-involvement (*type = 3*) 
...
2. Agent **Relational Mobility**, which constrains social dynamics to be:
  + non-existant; agents receive random input in each timestep. I.e., the tensor product model with random input at each timestep. (*relational_mobility = 1*)
  + static; agents communicate in a fully connected network with consistent influence overtime. I.e., the tensor product and social influence models, with no network rewiring (*relational_mobility = 2*) 
  + dynamic, with high relational mobility; agents strengthen ties to those with similar ideologies and sever ties to those that they disagree with. I.e., tensor product and social infuence models with a dynamic influence network (*relational_mobility = 3*)

Set 'desired_output' in the following code chunk and then run all. 

```{r}
desired_output <-"Fig_2"
```

## Output Options 

1. "Fig_2" - *Trajectories of the four cognitive styles* 
  + Illustrates the consequences of agent cognitive style in the tensor product model, by plotting the dot product of each agent's output and an ideology a over a run. 
  + Agent cognitive type varied (1:4) with relational_mobility set to 1 

2. "Fig_3" - *Trajectories of opinion dynamcs with communication* 
  + Adds the social influence model to the tensor product model, with a static influence network.
  + Plots the cosine similarity of agent's output and an ideology a over a run. 
  + Agent cognitive type varied (1:4) with relational_mobility set to 2 
  
3. "Fig_4" - *Trajectories of opinion dynamics with high relational mobility* 
  + The tensor product model with social influence in a dynamic influence network 
  + Again, plots the cosine similarity of agent's output and an ideology a over a run. 
  + Agent type varied (1:4) with relational_mobility set to 3

4. "Fig_s2" - *Mean number of opinion clusters and standard deviation of opinion similarities in simulated opinion dynamics for Type 2 (Ideological Interpreter) * 
  + Plots the variability in social network structure and opinion dynamics with and without relational mobility for type 2 agents.
  + Agent type is set to 2, relational_mobility is set sequentially to 2 (no mobility) and 3 (high relational mobility)
  + (a warning, this is slow to run)

5. "Fig_s3" - *Mean number of opinion clusters and standard deviation of opinion similarities in simulated opinion dynamics for Type 4 (No Ideological Mindset) * 
  + As in fig_s2 but for type 4 agents. 
  + (another warning, this is slow to run)

## CODE

```{r}
# First, make sure desired_output is actually set 
if (!exists("desired_output")) { 
  stop("set desired_output!")           # if empty, shout! 
}
...
# Set Up  ---------------------------------------------------------------------
 ...
 library(tidyverse)   
 library(ggpubr)  # used to arrange plots 
...
if (desired_output == "Fig_s2" | desired_output == "Fig_s3") {
...
w_i <- .5 # the learning rate
# 0 < w_i < 1, where larger values = slower learning

op <-  10  # the opposition strength
...
if (desired_output == "Fig_2" | 
    desired_output == "Fig_3" |
    desired_output == "Fig_4") {
  while (types_run < 5) {
    ...
    for (simnum in 1:simnum_N) {
      ...
      for (t in 1:T) {  # in each timestep
        ...
        for (agent_i in 1:agent_N) {  # each agent goes through
          ...
          # attributes source via M 
          s[agent_i, ] <- M[, , agent_i] %*% e[agent_i, ]  
          # forms an episodic memory 
          E <- s[agent_i, ] %*% t(e[agent_i, ])  
          # updates their long-term memory 
          M[, , agent_i] <-  w_i * M[, , agent_i] +  (1 - w_i) * E 
          # generates output 
          output[agent_i, ] <- 
            # first, filtering product of self-opinion and memory  
            C[, , agent_i] %*% normv(as.vector(t(s0[agent_i, ]) %*%  M[, , agent_i]))   
          # then saving it to feed into the Friedkin et al. model 
        ...
        if (change_ties == TRUE) {
          if (change_severe == TRUE) {
            for (i in 1:agent_N) {
              ...
              W[i, i] <- stubborn[i] # self-influence is trait, so stays same over runs 
            } # end i 
...
if (desired_output == "Fig_3" | desired_output == "Fig_4") {
...
if (desired_output == "Fig_s2" | desired_output == "Fig_s3" ) {
  ...
  for (c in 1:conditions) {
    ...
    mean_cluster <- stats::aggregate(hold$cluster, 
                                     ...
                                     FUN = "mean")
    if (c == 1) {
      mean_cluster$mobility <- "no mobility"
      } else { 
      mean_cluster$mobility <- "high relational mobility"
    }
...
### Qs? 
---> email ellep@student.unimelb.edu.au 
